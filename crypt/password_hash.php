 ...
 * 注意，该常量会随着 PHP 加入更新更高强度的算法而改变。 所以，使用此常量生成结果的长度将在未来有变化。
 ...
 * 就像以上提及的，在 PHP 7.0 提供 salt选项会导致废弃（deprecation）警告。 
 ...
 * 任何内核中的新算法必须在经历一次 PHP 完整发行才能成为默认算法。 比如，在 PHP 7.5.5 中添加的新算法，
 * 在 PHP 7.7 之前不能成为默认算法 （由于 7.6 是第一个完整发行版）。 但如果是在 7.6.0 里添加的不同算法，
 * 在 7.7.0 里也可以成为默认算法。
